...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """
        Returns the best threshold and Gini split score for one feature
        """
        ...
        if thresholds.size == 0:
            return None, np.inf
        left_mask = feature_values[:, None] > thresholds[None, :]
        class_mask = targets[:, None] == classes[None, :]
        left_class = left_mask[:, :, None] & class_mask[:, None, :]
        left_counts = left_class.sum(axis=0).astype(float)
        left_totals = left_mask.sum(axis=0).astype(float)
        left_totals_safe = np.where(left_totals == 0, 1, left_totals)
        left_probs = left_counts / left_totals_safe[:, None]
        gini_left = 1 - np.sum(left_probs ** 2, axis=1)
        right_mask = ~left_mask
        right_class = right_mask[:, :, None] & class_mask[:, None, :]
        right_counts = right_class.sum(axis=0).astype(float)
        right_totals = right_mask.sum(axis=0).astype(float)
        right_totals_safe = np.where(right_totals == 0, 1, right_totals)
        right_probs = right_counts / right_totals_safe[:, None]
        gini_right = 1 - np.sum(right_probs ** 2, axis=1)
        n = feature_values.size
        gini_avg = (
            left_totals / n * gini_left +
            right_totals / n * gini_right
        )
